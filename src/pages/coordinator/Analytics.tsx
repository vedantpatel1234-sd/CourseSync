...
export default function CoordinatorAnalytics() {
  ...
  const exportCSV = () => {
    const rows = [
      ['Instructor', 'Hours Assigned', 'Max Hours'],
      ...instructors.map(i => [i.full_name, i.hoursAssigned.toString(), i.maxHours.toString()])
    ]
    ...
    a.href = url
    a.download = 'coordinator-analytics.csv'
    a.click()
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: '#6B6B80' }}>
            Winter 2026 scheduling overview — read only
          </p>
        </div>
        <button
          onClick={exportCSV}
